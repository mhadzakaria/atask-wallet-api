# frozen_string_literal: true

FactoryBot.define do
  factory :transaction do
    amount { Faker::Commerce.price }

    trait :deposit do
      type { 'Deposit' }
      association :target_wallet, factory: %i[wallet for_user]
    end

    trait :withdraw do
      type { 'Withdraw' }
      association :source_wallet, factory: %i[wallet for_user]
    end

    trait :transfer do
      type { 'Transfer' }
      association :source_wallet, factory: %i[wallet for_user]
      association :target_wallet, factory: %i[wallet for_team]
    end
  end
end

# == Schema Information
#
# Table name: transactions
#
#  id               :integer          not null, primary key
#  amount           :decimal(, )
#  type             :string
#  created_at       :datetime         not null
#  updated_at       :datetime         not null
#  source_wallet_id :integer
#  target_wallet_id :integer
#
# Indexes
#
#  index_transactions_on_source_wallet_id  (source_wallet_id)
#  index_transactions_on_target_wallet_id  (target_wallet_id)
#
# Foreign Keys
#
#  source_wallet_id  (source_wallet_id => wallets.id)
#  target_wallet_id  (target_wallet_id => wallets.id)
#
